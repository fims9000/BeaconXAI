...
def _collect_features(
    ...
    return feats, y, q_used, censored


def _fit_score(x_val: np.ndarray, y_val: np.ndarray, x_test: np.ndarray, seed: int) -> np.ndarray:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="HAR K0 sweep for compact risk")
    p.add_argument("--dataset-root", default="./data")
    p.add_argument("--q-values", default="8,16")
    p.add_argument("--k0-values", default="4,6,8")
...
def main() -> None:
    ...
    k0_values = [int(v) for v in args.k0_values.split(",") if v.strip()]
    print(f"[k0-sweep] q={q_values} k0={k0_values}", flush=True)
    ...
    print(f"[k0-sweep] train cnn epochs={args.cnn_epochs}", flush=True)
    clf = train_1dcnn(
        x_tr,
        y_tr,
    ...
    out_rows: list[dict] = []
    for k0 in k0_values:
        print(f"[k0-sweep] evaluate k0={k0}", flush=True)
        cfg = replace(base_cfg, k0=k0)
        ...
        nm = sorted((r for r in rows_test if r.method == "negative_margin" and r.q_max == 0), key=lambda z: z.sample_id)
        y_base = np.array([r.is_error for r in nm], dtype=np.int64)
        s_base = _rank_norm(np.array([r.risk_score for r in nm], dtype=np.float64))
        p10_base, r10_base = _precision_recall_at_frac(y_base, s_base, 0.10)
        ...
        for q in q_values:
            ...
            if fv is None or ft is None:
                continue
            feats_v, yv, _, _ = fv
            feats_t, yt, q_used_t, cens_t = ft
            base_t = feats_t["neg_margin"]
            _append_row(
                out_rows,
                k0,
                q,
            ...
            for name, keys in method_defs.items():
                ...
                xt = np.stack([feats_t[k] for k in keys], axis=1)
                st = _fit_score(xv, yv, xt, args.seed + 100 + q + k0)
                _append_row(
                    out_rows,
                    k0,
                    q,
