...
def _collect_margin_ce(
    ...
    x_ce = _rank_norm(np.array([lm[i].counter_evidence_gain for i in ids], dtype=np.float64))
    X = np.stack([x_nm, x_ce], axis=1)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Stress HAR for mixed+CE vs negative_margin")
    p.add_argument("--dataset-root", default="./data")
    p.add_argument("--val-frac", type=float, default=0.2)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--q-values", default="16,64")
    ...
    p.add_argument("--out", default="./outputs_composite/har_stress_mixed_ce_full.csv")
    return p.parse_args()
...
def main() -> None:
    ...
    out_rows: list[dict] = []
    for stress_mode in ["clean", "noise", "time_mask", "channel_dropout"]:
        ...
        for q in q_values:
            ...
            dv = _collect_margin_ce(rows_va, local_va, q)
            dt = _collect_margin_ce(rows_te, local_te, q)
            if dv is None or dt is None:
                continue
            Xv, yv, _ = dv
            Xt, yt, _ = dt

            w = _fit_weights(Xv, yv, args.n_calib_trials, args.seed + q + len(stress_mode) * 17)
            s_ce = Xt @ w
            s_nm = Xt[:, 0]
            ...
            p5_ce, _ = _precision_recall_at_frac(yt, s_ce, 0.05)
            p10_ce, r10_ce = _precision_recall_at_frac(yt, s_ce, 0.10)
            lo, hi, frac = _bootstrap_delta(yt, s_ce, s_nm, args.n_bootstrap, args.seed + q + len(stress_mode) * 101)
            ...
            out_rows.append(
                {
                    ...
                    "auroc_mixed_ce": a_ce,
                    "delta_auroc": a_ce - a_nm,
                    "auprc_negative_margin": p_nm,
                    "auprc_mixed_ce": p_ce,
                    "delta_auprc": p_ce - p_nm,
    ...
    with out.open("w", newline="", encoding="utf-8") as f:
        wr = csv.DictWriter(
            ...
            fieldnames=[
                "stress_mode",
                "q_max",
                "auroc_negative_margin",
                "auroc_mixed_ce",
                "delta_auroc",
                "auprc_negative_margin",
                "auprc_mixed_ce",
                "delta_auprc",
                "p5_negative_margin",
                "p5_mixed_ce",
