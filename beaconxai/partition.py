...
def split_component_channel_first(comp: Component) -> list[Component]:
    t_len, c_len = comp.shape
    if t_len <= 1 and c_len <= 1:
        return []
...
def make_initial_partition_channel_time(t_steps: int, channels: int, k0: int) -> list[Component]:
    if k0 < 1:
        raise ValueError("k0 must be >= 1")
    root = Component(cid="root", t0=0, t1=t_steps, c0=0, c1=channels)
    leaves: list[Component] = [root]
    ...
    while len(leaves) < k0 and heap:
        item = heapq.heappop(heap)
        comp = item.comp
        children = split_component_channel_first(comp)
        if not children:
            continue
